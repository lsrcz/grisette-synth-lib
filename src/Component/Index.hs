...
class SOrd i => Index i where
  ...
  mkInternalIndex numOfInputs internalIndex = mkInputIndex (numOfInputs + internalIndex)
...
instance {-# OVERLAPPABLE #-} (Num i, SOrd i) => Index i where
...
instance {-# OVERLAPPABLE #-} (Num i, Ord i) => CIndex i where
